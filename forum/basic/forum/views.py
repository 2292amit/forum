...
def post(request, ptype, pk):
    """Display a post form."""
    action = reverse("basic.forum.views.%s" % ptype, args=[pk])
    if ptype == "new_thread":
...
def new_thread(request, pk):
    ...
    return HttpResponseRedirect(reverse("basic.forum.views.forum", args=[pk]))
...
def reply(request, pk):
    ...
    if p["body"]:
        ...
        increment_post_counter(request)
    return HttpResponseRedirect(reverse("basic.forum.views.thread", args=[pk]) + "?page=last")
